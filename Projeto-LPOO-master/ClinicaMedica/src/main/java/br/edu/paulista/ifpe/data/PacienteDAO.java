...
public class PacienteDAO {
	...
	public Paciente buscarTodos(int idPaciente) throws Exception {
		...
		String sql = "SELECT " + "   p.id, " + "   p.nome, " + "   p.cpf, " + "   p.telefone, " + "   p.nascimento, "
				+ "   p.sexo, " + "   p.estado_civil, " + "   p.nome_mae, " + "   p.cidade, " + "   p.bairro, "
				+ "   h.observacoes, " + "   h.proximos_passos, " + "   r.nome AS nome_remedio " + "FROM "
				+ "   paciente p " + "INNER JOIN " + "   historico h ON p.id = h.id_paciente " + "INNER JOIN "
				+ "   historico_remedio hr ON h.id = hr.id_historico " + "INNER JOIN "
				+ "   remedio r ON hr.id_remedio = r.id " + "WHERE p.id = ? " + 
				"ORDER BY " + "   p.nome";
